    ...
    match s.find(separator) {
        ...
        Some(index) => {
            match (T::from_str(&s[..index]), T::from_str(&s[index + 1..])) {
                (Ok(l), Ok(r)) => Some((l, r)),
                _ => None // use the catch-all _ here because if only one side fails, it's still bad
            }
        }
    }
}
...
fn pixel_to_point(bounds: (usize, usize),
    ...
    -> Complex<f64>
{
    let (width, height) = (bottom_right.re - upper_left.re,
                           upper_left.im - bottom_right.im);
    Complex {
    ...
    for row in 0..bounds.1 {
        for col in 0..bounds.0 {
            ...
            pixels[row * bounds.0 + col] = 
                match escape_time(point, 255) { // limit is 1 byte in size
                    None => 0,
...
fn main() {
    ...
    // This was for the single-threaded version
...

    {
        ...
        crossbeam::scope( |spawner| {
            for (i, band) in bands.into_iter().enumerate() {
                ...
                let band_upper_left = pixel_to_point(bounds, (0, top), upper_left, bottom_right);
                let band_bottom_right = pixel_to_point(bounds, (bounds.0, top + height), upper_left, bottom_right);
